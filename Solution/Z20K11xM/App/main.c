...
void TIM_IRQRLD(void)
{
    ...
    if (((ABS(CCV0 - CCV2) < Svm.StableScale) && (ABS(CCV0 - CCV2) > 0)) || ((ABS(CCV0 - CCV4) < Svm.StableScale) && (ABS(CCV0 - CCV4) > 0)) || ((ABS(CCV2 - CCV4) < Svm.StableScale) && (ABS(CCV2 - CCV4) > 0)))
    {
        while (1)
            ;
    }
...
void main(void)
{
    ...
    PORT_PinmuxConfig(PORT_A, GPIO_11, PTC8_GPIO);
    GPIO_SetPinDir(PORT_A, GPIO_11, GPIO_OUTPUT);
    GPIO_ClearPinOutput(PORT_A, GPIO_11);
