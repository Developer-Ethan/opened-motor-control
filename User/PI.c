 ...
 *
 */
inline int16_t PID_Ctr(PI_DEF *pPi, int32_t error)
{
    int32_t I_Out;
    int32_t Pi_Out;
    int32_t Integral;
    int32_t Proportion;
		...
		OutMin = pPi->OutMin << 15u;
		OutMax = pPi->OutMax << 15u;
    ...
    Proportion = (pPi->Error * pPi->Pid_Kp) >> SHIFT_15BITS;
    Proportion = Data_Limit(Proportion, 16);

    Pi_Out = (pPi->I_Out >> 15u) + Proportion;
