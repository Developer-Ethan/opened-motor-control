...
#ifndef Z20K11XM_I2C_H
#define Z20K11XM_I2C_H
...
typedef enum
{
    I2C_SLV_STOP_DET_WHE_ADDRESSED = 0U,                    /*!< In slave mode:I2C generates the STOP_DETECT interrupt flag no matter slave address match or not. */
    I2C_SLV_STOP_DET_ADDRESSED                              /*!< In slave mode:I2C generates the STOP_DETECT interrupt flag only slave addresse matches*/
...
typedef enum
{
    I2C_MST_STOP_DET_WHE_ACTIVE = 0U,                       /*!< In master mode:I2C generates the STOP_DETECT interrupt flag no matter master's state. */
    I2C_MST_STOP_DET_ACTIVE                                 /*!< In master mode:I2C generates the STOP_DETECT interrupt flag only when I2C master is active.*/
} I2C_StopDetActive_t;
...
typedef enum
{
    ...
    I2C_GEN_NACK_AFTER_RCV                                  /*!< I2C generates NACK after data byte is received. Then this data will be aborted,not pushed into RX FIFO. */
...
typedef enum
{
    ...
    I2C_GEN_CALL_ADDR = 2U,                                 /*!< I2C will launch a General Call (transfer data is 0x00), only writes may be performed.Any read option will result asserting ERROR_ABORT flag. */
    I2C_START_BYTE = 3U,                                    /*!< I2C transfers START BYTE (transfer data is 0x01), should no slave ack. */
} I2C_MstCmdSelect_t;
...
typedef enum
{
    I2C_TX_BUFFER = 0U,                                     /*!< When TXFIFO_EMPTY_EN = 0: This bit is set to 1 when the transmit buffer is at or below the threshold value set in the I2C_TXFIFO_WATER_MARK register.*/
    I2C_TX_DATA_PATH                                        /*!< When TXFIFO_EMPTY_EN = 1: This bit is set to 1 when the transmit buffer is at or below the threshold value set in the I2C_TXFIFO_WATER_MARK register
...
typedef enum
{
    ...
    I2C_INT_TX_OVER,                                        /*!< Transmit FIFO above TX FIFO depth and CPU attempts to issue another I2C command by writing to the I2C_COMMAND_DATA register. */
    ...
    I2C_INT_RX_DONE,                                        /*!< The transmission done interrupt,only valid for I2C as a slave transmitter. */
    I2C_INT_RX_UNDER,                                       /*!< Receive FIFO underflow interrupt. */
    I2C_INT_RX_OVER,                                        /*!< Received FIFO overflow interrupt. */
    I2C_INT_RESTART_DET,                                    /*!< A RESTART signal generated interrupt,operating in slave mode and the slave is the addressed slave. */
    I2C_SCL_STUCK_AT_LOW,                                   /*!< SCL stuck at low interrupt. */
    I2C_INT_RX_FULL,                                        /*!< Received FIFO full interrupt. */
    I2C_INT_TX_EMPTY,                                       /*!< Transmit FIFO empty interrupt.
                                                              - When TXFIFO_EMPTY_EN = 0:
                                                              This bit is set to 1 when the transmit buffer is at or below the threshold value set in the I2C_TXFIFO_WATER_MARK register.
                                                              - When TXFIFO_EMPTY_EN = 1:
                                                              This bit is set to 1 when the transmit buffer is at or below the threshold value set in the I2C_TXFIFO_WATER_MARK register and the transmission of the
                                                              address/data from the internal shift register for the most recently popped command is completed. */
...
typedef enum
{
    ...
    I2C_TXFIFO_EMPTY_MASTER_HOLD,                           /*!< This bit indicates Tx FIFO is empty, and the the previous transferred command does not have Stop bit,so master hold the bus. */
    I2C_TXFIFO_EMPTY_SLAVE_HOLD,                            /*!< This bit indicates when the Tx FIFO is empty, slave will hold the bus until slave TX FIFO has data to Transmit for the read request. */
    I2C_RXFIFO_FULL_MASTER_HOLD,                            /*!< This bit indicates RX FIFO is full and receive new data,so master will hold the bus. */
    I2C_RXFIFO_FULL_SLAVE_HOLD,                             /*!< This bit indicates RX FIFO is full and receive new data,so slave will hold the bus. */
    I2C_STATUS_TFNF,                                        /*!< Transmit FIFO not full,means TX FIFO still has one or more empty loactions. */
    I2C_STATUS_TFE,                                         /*!< Transmit FIFO empty,means TX FIFO is completely empty. */
    I2C_STATUS_RFNE,                                        /*!< Receive FIFO not empty.When RX FIFO has one or more entries already. This bit will be set to 1. */
    I2C_STATUS_RFF,                                         /*!< Receive FIFO full.When RX FIFO is completely full, this bit will be set 1. */
    I2C_SDA_ERR_RECOVER_STUCK_LOW,                          /*!< SDA error recover from stucking at low phase.This bit is set when SDA fail to recover,and still stuck at low phase. */
    I2C_SLAVE_IS_DISABLED_UNDER_ACT,                        /*!< Slave is disabled under active state.When read as 1, it means slave is potentially disabled during slave transfer phase. */
    I2C_SLAVE_RX_DATA_DISCARD,                              /*!< Slave receive data discard.When read as 1, it means I2C slave is aborted with at least one data byte from I2C transfer due to MODULE_EN from 1 to 0. */
...
typedef enum
{
    ERR_GEN_CALL_NO_ACK = 0U,                               /*!< I2C in master mode sent a General Call and no slave on the bus acknowledged the General Call. */
    ERR_GEN_CALL_READ,                                      /*!< I2C in master mode sent a General Call but the user programmed the byte following the General Call to be a read from the bus */
    ERR_START_BYTE_ACK_DET,                                 /*!< Master has sent a START Byte and the START Byte was acknowledged (wrong behavior) */
    ERR_SBYTE_NORSTRT,                                      /*!< The start is disabled and the user is trying to send a START Byte */
    ERR_H_NO_RSTRT,                                         /*!< The restart is disabled and the user is trying to use the master to transfer data in high speed mode. */
    ERR_H_MCODE_ACK_DET,                                    /*!< Master is in High Speed mode and the High Speed Master code was acknowledged (wrong behavior). */
    ERR_7BIT_ADDR_NO_ACK,                                   /*!< Master is in 7-bit addressing mode and the address sent was not acknowledged by any slave */
    ERR_10BIT_ADDR1_NO_ACK,                                 /*!< Master is in 10-bit address mode and the first 10-bit address byte was not acknowledged by any slave. */
    ERR_10BIT_ADDR2_NO_ACK,                                 /*!< Master is in 10-bit address mode and the second address byte of the 10-bit address was not acknowledged by any slave. */
    ERR_10BIT_READ_NO_RSTRT,                                /*!< Will assert 1 when read 10 bit under EN_RESTART = 0. */
    ERR_DATA_NO_ACK,                                        /*!< This is a master-mode only bit. Master has received an acknowledgement for the address, but when it sent data byte(s) following the address,
                                                              it did not receive an acknowledge from the remote slave(s). */
    ERR_MASTER_ABRT,                                        /*!< This is a master-mode-only bit. Master has detected the transfer abort. */
    ERR_MASTER_DIS,                                         /*!< User tries to initiate a Master operation with the Master mode disabled */
    ERR_SLAVE_ARBLOST,                                      /*!< Slave lost the bus while transmitting data to a remote master.I2C_ERROR_STATUS[14] is set at the same time. */
    ERR_MASTER_LOST,                                        /*!< Master has lost arbitration, or if I2C_ERROR_STATUS[13] is also set, then the slave transmitter has lost arbitration. */
    ERR_SLAVE_READ_REQ,                                     /*!< When the processor side responds to a slave mode request for data to be transmitted to a remote master and user writes a 1 in CMD_MASTER_DIRECTION (bit 8) of I2C_COMMAND_REG register. */
    ERR_SLAVE_FLUSH_TXFIFO,                                 /*!< Slave has received a read command and some data exists in the TX FIFO so the slave issues a ERROR_ABORT interrupt to flush old data in TX FIFO. */
    ERR_SDA_LOW_TIMEOUT,                                    /*!< SDA stuch at low and timeout error. */
    ERR_STATUS_ALL
} I2C_ErrorStatus_t;
...
typedef struct
{
    ...
    I2C_Speed_t speedMode;                                  /*!< Specifies the I2C speed mode.
	                                                          ...
	                                                          - I2C_SPEED_HIGH: high speed (up to 1.6 Mb/s)
    ...
    ControlState_t restart;                                 /*!< Determines whether RESTART conditions may be sent when acting
                                                              as a master.
                                                              - ENABLE: enable RESTART condition
                                                              - DISABLE: disable RESTART condition
                                                              When the RESTART is disabled, the I2C module master is incapable of performing the following functions:
...
typedef struct
{
    uint32_t sdaReceiveHoldTime;                            /*!< Sets the required SDA hold time in units of I2C function clock period, when I2C module acts as a reciever. 
                                                                 SDA data hold time after SCL negedge. */
    uint32_t sdaTransmitHoldTime;                           /*!< Sets the required SDA hold time in units of I2C function clock period, when I2C module acts as a transmitter. 
                                                                 SDA data hold time after SCL negedge. */
} I2C_SdaHoldTime_t;
...
typedef struct
{
    uint32_t I2C_DMA_TransmitReqLevel;                      /*!< DMA transmit data level, the valid range is 0 - 3 */
    uint32_t I2C_DMA_RecvReqLevel;                          /*!< DMA Receive data level, the valid range is 0 - 3 */
} I2C_DmaConfig_t;
 ...
 *             - ERR_GEN_CALL_NO_ACK
 *             - ERR_GEN_CALL_READ
 *             - ERR_START_BYTE_ACK_DET
 *             - ERR_SBYTE_NORSTRT
 *             - ERR_H_NO_RSTRT
 *             - ERR_H_MCODE_ACK_DET
 *             - ERR_7BIT_ADDR_NO_ACK
 *             - ERR_10BIT_ADDR1_NO_ACK
 *             - ERR_10BIT_ADDR2_NO_ACK
 *             - ERR_10BIT_READ_NO_RSTRT
 *             - ERR_DATA_NO_ACK
 *             - ERR_MASTER_ABRT
 *             - ERR_MASTER_DIS
 *             - ERR_SLAVE_ARBLOST
 ...
 *            are flushed due to ERROR_ABORT or Slave Transmitter interrupt.
 *
 * @param[in]  i2cNo: Select the I2C port, should be I2C0_ID, I2C1_ID.
 *
 * @return none
