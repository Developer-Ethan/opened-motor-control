...
%% 电机参数
Rs_realvalue = 28;%定子电阻  ou
Ld_realvalue = 2.7e-3;% Ld  H
Lq_realvalue = 2.7e-3;% Ld  H
%Lq_realvalue = 10.8e-3;% Lq  H  
Np = 1;%极对数
Lam_vrms_krpm = 1.47; %1000rpm的反电动势有效值 7.85
Lam_realvalue = ((Lam_vrms_krpm*1.414)/1.732)/(2*pi*(1000/(60/Np)));%磁链，对应相电压幅值
J_realvalue = 2.47e-4;%转动惯量
Friction = 0.003;%摩擦系数 
Tn = 0.012;%额定力矩 NM
fn = 1700;%额定转速 Hz 
In = 0.35;%额定线电流有效值 Arms
Un = 220;%额定线电压有效值 Vrms
...
%% 磁链参数整定
LamObserverGain = 0.4;
LamObserverGain_Q = Q15*LamObserverGain;
